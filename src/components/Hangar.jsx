...
import { GRADE_COLORS } from "../data/grades.js";
...
export default function Hangar({ currentUserId }) {
  ...
  const completedKits = kits.filter(k => progress[k.id] === "complete" && !favourites.includes(k.id));
  const inProgressKits = kits.filter(k => progress[k.id] === "inprogress" && !favourites.includes(k.id));
  const backlogKits = kits.filter(k => progress[k.id] === "backlog" && !favourites.includes(k.id));
  ...
  const KitCard = ({ kit }) => {
    ...
    return (
      <div
        ...
        onClick={() => navigate(`/kit/${kit.grade.toLowerCase()}-${kit.scale.replace("/", "")}-${kit.name.toLowerCase().replace(/\s+/g, "-")}`)}
      ...
      >
        ...
        <div style={{ width: "100%", aspectRatio: "1", background: "rgba(0,0,0,0.3)", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{
            ...
            fontFamily: "'Share Tech Mono',monospace", fontSize: "0.5rem", color: colors.accent,
            padding: "2px 8px", letterSpacing: "1.5px",
          }}>
            {kit.grade}
          </div>
          ...
          {kitPhotos.length > 0 && (
            <div style={{
              ...
              fontFamily: "'Share Tech Mono',monospace", fontSize: "0.45rem", color: "#ccc",
        ...
        <div style={{ padding: "10px 12px" }}>
          <div style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: "0.75rem", fontWeight: 600, color: "var(--text)", letterSpacing: "0.5px", lineHeight: 1.3 }}>
            {kit.name}
          </div>
          <div style={{ fontFamily: "'Share Tech Mono',monospace", fontSize: "0.5rem", color: "var(--text-dim)", letterSpacing: "1px", marginTop: 2 }}>
            {kit.grade} · {kit.scale}
          </div>
        </div>
  ...
  const Section = ({ title, icon, kits: sectionKits, color }) => {
    ...
    return (
      <div style={{ marginBottom: 32 }}>
        <div style={{
          fontFamily: "'Share Tech Mono',monospace", fontSize: "0.7rem", letterSpacing: "2px",
          color: color || "var(--text-dim)", marginBottom: 12, display: "flex", alignItems: "center", gap: 8,
        }}>
          {icon} {title}
          <span style={{ fontSize: "0.55rem", color: "var(--text-dim)", letterSpacing: "1px" }}>
  ...
  return (
    <div className="page-wrap" style={{ maxWidth: 1000, margin: "0 auto", padding: "40px 20px 80px" }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 200 }}>
          <div style={{ fontFamily: "'Orbitron',sans-serif", fontSize: "1.2rem", fontWeight: 700, color: "var(--text)", letterSpacing: "1px" }}>
            {profile.display_name || profile.username}
          </div>
          <div style={{ fontFamily: "'Share Tech Mono',monospace", fontSize: "0.6rem", color: "var(--text-dim)", letterSpacing: "1px", marginTop: 2 }}>
            @{profile.username}
            {memberDate && <span> · Member since {memberDate}</span>}
          </div>
          {profile.bio && (
            <div style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: "0.8rem", color: "var(--text)", marginTop: 8, lineHeight: 1.5, maxWidth: 500 }}>
              {profile.bio}
            </div>
          )}
        </div>
        ...
        {is_owner && (
          <button
            ...
            style={{
              ...
              padding: "8px 16px", cursor: "pointer", letterSpacing: "1px",
              whiteSpace: "nowrap",
            }}
          >
      ...
      <div style={{
        ...
        {[
          ...
          <div key={s.label} style={{
            flex: "1 1 100px", background: "rgba(0,0,0,0.25)", border: "1px solid rgba(255,255,255,0.06)",
            padding: "12px 10px", textAlign: "center",
          }}>
            <div style={{ fontFamily: "'Orbitron',sans-serif", fontSize: "1.1rem", fontWeight: 700, color: s.color }}>
              {s.value}
            </div>
            <div style={{ fontFamily: "'Share Tech Mono',monospace", fontSize: "0.45rem", color: "var(--text-dim)", letterSpacing: "1.5px", marginTop: 4 }}>
              {s.label}
            </div>
          </div>
        ))}
      ...
      {kits.length === 0 && (
        <div style={{ textAlign: "center", padding: "60px 20px" }}>
          <div style={{ fontFamily: "'Share Tech Mono',monospace", fontSize: "0.8rem", color: "var(--text-dim)", letterSpacing: "1.5px" }}>
      ...
      {gallery_posts.length > 0 && (
        <div style={{ marginTop: 40 }}>
          <div style={{
            fontFamily: "'Share Tech Mono',monospace", fontSize: "0.7rem", letterSpacing: "2px",
            color: "#ff6600", marginBottom: 12,
          }}>
            📸 GALLERY POSTS
            <span style={{ fontSize: "0.55rem", color: "var(--text-dim)", letterSpacing: "1px", marginLeft: 8 }}>
              ...
              return (
                ...
                >
                  ...
                  <div style={{
                    ...
                    <div style={{ fontFamily: "'Share Tech Mono',monospace", fontSize: "0.45rem", color: "#fff", letterSpacing: "0.5px" }}>
                      {post.kit_name}
                    </div>
                    <div style={{ fontFamily: "'Share Tech Mono',monospace", fontSize: "0.4rem", color: "rgba(255,255,255,0.5)", letterSpacing: "0.5px", marginTop: 2 }}>
                      ♥ {post.likes || 0}
                    </div>
                  </div>
                </div>
